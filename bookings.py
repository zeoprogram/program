...
from models.booking import Booking, BookingCreate, BookingStatusUpdate, MetaResponse
...
def booking_code() -> str:
    return f"HG-{datetime.now(timezone.utc).year}-{secrets.token_hex(3).upper()}"
...
async def list_bookings(
    ...
    documents = await db.bookings.find(query).sort("created_at", -1).to_list(200)
...
async def track_bookings(q: str = Query(min_length=2, max_length=100)):
    escaped = q.strip()
    documents = await db.bookings.find(
        {"$or": [{"code": escaped.upper()}, {"whatsapp": escaped}]}
...
async def get_booking(booking_id: str):
    document = await db.bookings.find_one({"$or": [{"id": booking_id}, {"code": booking_id.upper()}]})
    if not document:
...
async def update_booking_status(booking_id: str, input: BookingStatusUpdate):
    ...
    result = await db.bookings.update_one(
        {"id": booking_id},
        {"$set": {"status": input.status, "updated_at": updated_at}},
    )
